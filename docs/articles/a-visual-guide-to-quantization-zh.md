...
一个数值通常表示为浮点数（floating point number），也就是带小数点的正数或负数。
...
在实践中，我们并不需要把整个 FP32 的范围 $[-3.4\mathrm{e}38, 3.4\mathrm{e}38]$ 全部映射进 INT8。我们真正要做的，只是把“当前数据，也就是模型参数本身”的取值范围映射进 INT8。
...
对称量化会把原始浮点值的范围，映射到一个对称且以 0 为中心的量化空间里。前面的示意图里也能看出来：量化前后，范围都围绕 0 对称展开。
...
给定一组数值，我们取其中绝对值最大的那个数 $\alpha$ 作为映射范围。
...
我们首先计算缩放因子（scale factor）$s$。这里的 $b$ 表示量化位宽，在这个例子里取 8；$\alpha$ 则是最大绝对值。
...
与对称量化不同，非对称量化不再围绕 0 对称。它会把浮点范围中的最小值 $\beta$ 和最大值 $\alpha$，直接映射到量化范围里的最小值和最大值。
...
GGUF 的具体量化细节会随着位宽和实现不断演进，但其大体思路可以概括为：先把一层权重切分成若干“超级块（super blocks）”，每个超级块下面再包含若干“子块（sub blocks）”。随后，从这些块中分别提取缩放因子 $s$ 和 $\alpha$。
...
换句话说，GGUF 的块级量化，会使用超级块的缩放因子 $s_{\text{super}}$ 去量化子块的缩放因子 $s_{\text{sub}}$。
...
原论文里使用的符号略有不同。图中沿用了本文前面一直使用的 $\alpha$ 记号；同时，这里的 $\beta$ 也不是前面零点量化里的 $\beta$，而是“权重绝对值的平均值”。
...
同时，它还会记录一个 $\beta$，也就是权重绝对值的平均值，供后续反量化使用。
...
同样地，它还会记录一个 $\alpha$，也就是激活值的最大绝对值，后面反量化时会用到。

#### 反量化

因为前面已经记录了激活值的最大绝对值 $\alpha$，以及权重绝对值的平均值 $\beta$，所以这些量都可以用于把输出激活值重新缩放回原始精度附近。
...
它的思路是先压缩权重分布，再用绝对均值 $\alpha$ 来完成量化，最后把值四舍五入到 $-1$、$0$ 或 $1$ 三个离散点上。
