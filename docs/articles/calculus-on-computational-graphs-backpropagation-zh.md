...
除了在深度学习中的用途之外，反向传播在很多其他领域也是一种非常强大的计算工具，从天气预报到数值稳定性分析，都能见到它的身影——只是它们使用的是不同的名字。事实上，这个算法在不同学科中已经重新发明了至少几十次（参见 Griewank, 2010）。更通用、也与具体应用无关的名字是“反向模式微分”（reverse-mode differentiation）。
