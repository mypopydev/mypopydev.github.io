...
除了在深度学习中的用途之外，反向传播在很多其他领域也是一种非常强大的计算工具，从天气预报到数值稳定性分析，都能见到它的身影——只是它们使用的是不同的名字。事实上，这个算法已经在不同学科中被重新发明了至少几十次。更通用、也与具体应用无关的名字是“反向模式微分”（reverse-mode differentiation）。
...
导数比你想象得便宜。这是我希望你从这篇文章里带走的最核心的一点。事实上，它们便宜得有点反直觉，以至于我们这些人类不得不一遍又一遍地重新发现这个事实。在深度学习里，理解这一点非常重要；在其他很多领域里，知道这一点也同样极其有用，甚至在这些领域尚未形成共识时更是如此。
