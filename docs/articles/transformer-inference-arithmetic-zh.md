...
这其实又回到了 KV cache 那一节里已经出现过的同一个硬件比值。对 A100 来说，从内存带宽受限切换到 FLOPs 受限的临界 batch size 大约就是：

$$
\frac{A_f}{A_{\text{bm}}} = 208
$$

这里按前文使用的硬件参数 $312\mathrm{e}12 / 1.5\mathrm{e}12 = 208$ 计算；原文该处分子分母写反了，但数值显然意在表达这个比值。
