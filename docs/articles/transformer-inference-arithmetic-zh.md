...
第一个 `2` 表示两组向量，也就是 `k` 和 `v`。每一层都要各存一份，而每份本身又是一个 `n_heads \times d_head` 的矩阵。再乘上第二个 `2`，是因为本文始终假设使用 16 bit 格式，所以每个值占 2 字节。
...
把这些数代到一个 52B 参数模型里，比如 Anthropic 论文中那类 `d_model = 8192`、`n_layers = 64` 的配置，就可以开始估算量级了。
...
pipeline parallel 还有一个特点：它吃不满内存带宽。不过如果你本来就是 FLOPs 受限，这倒也未必是坏事。真正让 pipeline parallel 更有优势的地方在于通信。pipeline parallel 每个加速器只需要通信一个 `d_model` 大小的向量，而 tensor/model parallel 则要在每一层都通信 `N \cdot d_{\text{model}}` 量级的数据，其中 `N` 是加速器数量。
...
MLP 层本质上也差不多。就像我们用 `W_o` 把多头 attention 的结果投影回一个 `d_model` 长度的向量一样，MLP 会用：
...
通信项则来自模型并行小节里的结论：每一层里有 4 次通信，作者还把 `(N - 1)/N` 近似成了 `1`。每次通信的内容是一个 `d_model` 大小的向量，对 batch size 为 `B` 的情形要整体一起发，所以再乘上 `B` 与每个元素 2 字节的开销，最后除以通信带宽。
...
- 其他部分
  ...
  - 顶部还有一层线性层和 softmax，对应输出 embedding，也就是 unembedding / de-embedding / `embedding^{-1}`。
  - 原始 Transformer 还会有 cosine absolute positional encoding，本质上只是对 token embedding 做加法。

把这些 FLOPs 都加起来之后，再代入 `d_model = 8192` 的模型，可以得到大约 `100B` FLOPs。更具体地说，`103,079,215,104 / 2 \approx 51.5B`。它比 52B 略小一点，是因为 token embedding 与 unembedding 本身也接近 10 亿参数。
...
还有一个值得注意的点是：模型越大，这些中间操作所占的时间比例会下降得很快。因为它们的内存规模大体按 `d_{\text{model}}` 增长，而主 FLOPs 成本按 `d_{\text{model}}^2` 增长，而且还是逐层累积。论文里的例子只是一个 336M 参数模型，对应 `d_model = 1024`、`n_layers = 24`。

作者把论文里 “Ours” 一列中所有内存带宽受限的项，包括 element-wise operators，都加了一遍，得到这些中间步骤总共会占到 `43%` 的时间。但在 52B 模型里，`d_model` 大了 8 倍，于是这些操作的重要性就会迅速下降。

更具体地说，这些内存带宽受限的中间操作，因为处理的是 `d_model` 长度的向量，所以时间大约只会放大 8 倍；可与此同时，主矩阵乘法的 FLOPs 时间会放大 64 倍。因此如果沿用那篇论文里的优化思路，那么在 52B 模型推理里，前面没有算进延迟公式的这些中间操作，大约只会占总延迟的 `5%` 左右。
...
因为作者只想用 2 张 GPU，所以他选了一个 13B 参数模型来跑 FasterTransformer。这个模型有 40 层、40 个 attention head，每个 head 的维度是 128，因此 `d_model = 5120`。对应的 profile 截图作者放在[这个 Google Slides](https://docs.google.com/presentation/d/17PMhlxB3hZYBLqf2mych9CL8kyyssIEvJHOscpqTo1k/edit?usp=sharing) 里，其中还藏着不少可能值得单开一篇文章的话题。
