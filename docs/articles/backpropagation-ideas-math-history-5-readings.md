...
其中 $\mathrm{Pa}(j)$ 表示节点 $v_j$ 的父节点集合。前向传播对应的是按拓扑顺序依次计算每个节点的数值；反向传播对应的是在损失节点 $L$ 已知的前提下，反向计算每个节点对最终损失的敏感度。从自动微分的角度看，这样一张图也可以理解为对求值过程的记录，也可以视为一种计算轨迹（computation trace）；在较早的文献里，它也常被称为 Wengert list。
