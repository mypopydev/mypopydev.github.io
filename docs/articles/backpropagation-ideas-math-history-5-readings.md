...
![图 3：反向模式的效率优势](../assets/images/articles/backpropagation/fig_efficiency_comparison.svg)
...
![图 4：共享节点的梯度汇聚](../assets/images/articles/backpropagation/fig_shared_node_gradient_sum.svg)

如果把"共享路径"这件事再往前推一步，那么更典型的图形其实是共享节点本身。图 4 画的是同一个输入 $x$ 同时流向两条下游分支的情形。前向阶段看上去只是同一个值被重复使用；反向阶段则必须把两条分支返回的贡献相加，才能得到 $x$ 的总敏感度。这正是一般递推公式里求和号的来源，也是很多自动微分实现里梯度更新要写成累加而不是覆盖的根本原因。

![图 5：从逐条路径求和到共享节点处的因子化](../assets/images/articles/backpropagation/fig_path_factorization_sum.svg)

如果再把图做大一点，就会看到为什么反向传播必须这样组织计算。图 5 左侧表示"若把所有完整路径都逐条展开"，项数会随着前缀与后缀选择数相乘而迅速膨胀；右侧则表示反向传播真正做的事：不是沿每条完整路径各算一遍，而是在共享节点处先把可复用的局部贡献累积起来，再继续向前传递。也正因为如此，反向传播常被说成是在计算图上做动态规划：它复用的不是某一条完整路径，而是许多路径共享的中间子问题。
...
![图 6：从标量导数到局部 Jacobian 与 VJP](../assets/images/articles/backpropagation/fig_scalar_to_jacobian_vjp.svg)

图 6 想强调的不是又多了一套新规则，而是规则本身没有变：标量图里沿边传播的是局部导数，向量图和矩阵图里沿边传播的是局部线性映射。于是，前向模式更像是把一个输入侧方向往前推成 $Jr$，反向模式则是把一个输出侧敏感度往回拉成 $J^T \bar y$。从这个角度看，VJP 只是标量链式法则在线性代数语言里的自然推广。
...
因此，在线性层的小批量实现里，偏置梯度之所以常常需要沿批次维求和，并不是额外规定，而只是共享参数在广播场景下应用一般计算图递推公式后的自然结果：同一个节点若被多个后继同时使用，其对损失的总敏感度就必须把这些分支的贡献全部相加。图 7 将这一共享结构单独展开，以便把"参数共享"和"梯度归约"之间的关系看得更直接。虽然不同局部算子的前向形式各不相同，但它们的反向形式都可以统一地理解为局部 Jacobian 的转置作用。

![图 7：广播加法中偏置梯度沿批次维求和](../assets/images/articles/backpropagation/fig_broadcast_bias_sum.svg)

如图 7 所示，前向阶段并不是产生了 $B$ 份彼此独立的偏置，而是同一个参数向量 $b$ 被整批样本共享；反向阶段来自每一行输出的上游梯度都对这同一个参数产生贡献，因此最后必须沿批次维做归约。在线性层里常见的偏置梯度求和，正是这种共享节点结构在小批量张量记号下的直接表现。
...
#### 一个完整的数值算例：2-2-2 网络的一轮前向、反向与更新
为了先把这一轮迭代的全貌放在眼前，下面这张图把这个 2-2-2 网络的一次前向、反向与参数更新压到同一幅画面里：蓝色箭头表示前向算值，红色标注表示误差项与梯度回传，绿色区域表示一次梯度下降更新后的结果。

![图 8：一次完整训练迭代的数值流向（2-2-2 网络）](../assets/images/articles/backpropagation/fig_numeric_iteration_flow.svg)

图 8 的作用不是替代后面的具体计算，而是先给出一个"全局索引"：前向阶段从输入 $x$ 依次算出隐藏层激活、输出层激活与损失 $L$，反向阶段则从 $L$ 出发回传 $\delta^{(2)}$ 与 $\delta^{(1)}$，并据此得到两层参数梯度与一次更新。接下来出现的具体数值，只是在把图中的每一步逐项展开写清楚。

如果还想把上面的局部规则真正串成"一次完整训练迭代"，那么一个两层前馈网络的数值算例会很有帮助。参考 Matt Mazur 的经典手算例子，并改写成与本文一致的矩阵记号，设
...
其中两层都用 sigmoid 激活，损失采用单样本平方损失

$$
L = \frac{1}{2}\lVert a^{(2)} - t \rVert_2^2
$$

为了和本文前后的记号保持一致，这里把 bias 写成向量；它与原例中"同一层各神经元起始 bias 数值相同"的设置是等价的。

前向传播首先给出隐藏层与输出层的净输入和激活：
...
$$
L = \frac{1}{2}\left[(0.7514-0.01)^2 + (0.7729-0.99)^2\right] \approx 0.29837
$$

反向传播从输出层误差项开始：

$$
\delta^{(2)} = \frac{\partial L}{\partial z^{(2)}} = (a^{(2)}-t) \odot a^{(2)} \odot (1-a^{(2)}) \approx
\begin{bmatrix}
0.13850 \\
-0.03810
\end{bmatrix}
...
$$
\frac{\partial L}{\partial W^{(2)}} = \delta^{(2)} (a^{(1)})^T \approx
\begin{bmatrix}
0.08217 & 0.08267 \\
-0.02260 & -0.02274
\end{bmatrix}, \qquad
...
再把输出层误差通过转置权重拉回隐藏层，可得

$$
\delta^{(1)} = (W^{(2)})^T \delta^{(2)} \odot a^{(1)} \odot (1-a^{(1)}) \approx
\begin{bmatrix}
0.00877 \\
0.00995
\end{bmatrix}
...
$$
\frac{\partial L}{\partial W^{(1)}} = \delta^{(1)} x^T \approx
\begin{bmatrix}
0.000439 & 0.000877 \\
0.000498 & 0.000995
\end{bmatrix}, \qquad
...
这一行尤其值得慢一点看：隐藏层误差项并不是凭空出现的，而是先把两个输出节点返回的贡献按 $(W^{(2)})^T \delta^{(2)}$ 汇总起来，再乘上当前层 sigmoid 的局部导数。这正是"多条路径贡献求和"和"共享节点处梯度汇聚"的数值版本。

如果按梯度下降更新一次参数，

$$
W^{(l)}_{\text{new}} = W^{(l)} - \eta \frac{\partial L}{\partial W^{(l)}}, \qquad
b^{(l)}_{\text{new}} = b^{(l)} - \eta \frac{\partial L}{\partial b^{(l)}}
$$
...
$$
\begin{gathered}
W^{(1)}_{\text{new}} \approx
\begin{bmatrix}
0.1498 & 0.1996 \\
0.2498 & 0.2995
\end{bmatrix}, \qquad
b^{(1)}_{\text{new}} \approx
\begin{bmatrix}
0.3456 \\
0.3450
\end{bmatrix}
...
这里把 bias 也显式更新出来，以便和前面的通式保持一致；若只更新权重而暂不更新 bias，更新后的具体数值会略有不同。用这些新参数再做一次前向计算，损失约降到 $0.28047$，说明上面的梯度方向确实把参数往"更小损失"的方向推了一步。

这个完整算例可以把前面较抽象的公式真正落到数字上看清楚。第一，输出层误差项 $\delta^{(2)}$ 来自"损失对输出的敏感度"与"激活函数局部导数"的逐元素相乘。第二，隐藏层误差项 $\delta^{(1)}$ 不是重新从头求导，而是把输出层回传的贡献按路径汇总后再乘局部导数。第三，参数梯度始终保持同一个结构：当前层误差项乘以前一层激活，因此多层网络并没有引入新规则，只是把局部规则按层重复应用。
...
![图 9：单神经元梯度推导图](../assets/images/articles/backpropagation/fig_single_neuron_gradient.svg)
...
![图 10：前向模式 AD 与反向模式 AD 的 Jacobian 方向图](../assets/images/articles/backpropagation/fig_forward_vs_reverse_ad.svg)

图 10 的核心，是区分"谁在被传播"。前向模式 AD 从输入侧携带一个方向向量 $v$ 往前走，得到的是 $Jv$；反向模式 AD 从输出侧携带一个敏感度向量 $u^T$ 往回走，得到的是 $u^T J$。如果最终只有一个标量损失 $L$，那么这个 $u^T$ 可以看成 1，于是一次反向传播就能得到 $\partial L / \partial \theta$ 对所有参数的梯度。
...
![图 11：梯度消失示意图](../assets/images/articles/backpropagation/fig_vanishing_gradient.svg)
...
![图 12：信用分配（credit assignment）与逐层梯度分解](../assets/images/articles/backpropagation/fig_credit_assignment.svg)
...
如果还想知道现代框架的实现，可以先看 JAX 或 Dive into Deep Learning 关于自动微分的章节，再配合 George Assaad 于 2025 年写的 *Everything You Need to Know About Backpropagation*。前两者适合建立自动微分接口层面的基本认识，Assaad 的文章则进一步把"不必显式构造 Jacobian""线性层 VJP 的外积写法"以及 softmax-cross-entropy 梯度的实现视角串到 JAX `custom_vjp` 的代码框架里。
...
如果你想把"一次前向—反向—参数更新"按数字完整手算一遍，那么 Matt Mazur 的 *A Step by Step Backpropagation Example* 很值得顺手对照。它用一个 2-2-2 网络把前向数值、平方损失、输出层误差项、隐藏层误差项与参数更新按顺序全部摊开；若把它与本文前面新增的完整数值算例并读，会更容易看出两者只是记号层级不同，而不是原理不同。
...
21. [Matt Mazur (2015/2024) - A Step by Step Backpropagation Example](https://mattmazur.com/2015/03/17/a-step-by-step-backpropagation-example/)
