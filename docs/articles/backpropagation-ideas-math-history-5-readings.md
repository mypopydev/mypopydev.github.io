...
![图 12：一次完整训练迭代的数值流向（2-2-2 网络）](../assets/images/articles/backpropagation/fig_numeric_iteration_flow.svg)

图 12 的作用不是替代后面的具体计算，而是先给出一个“全局索引”：前向阶段从输入 $x$ 依次算出隐藏层激活、输出层激活与损失 $L$，反向阶段则从 $L$ 出发回传 $\delta^{(2)}$ 与 $\delta^{(1)}$，并据此得到两层参数梯度与一次更新。接下来出现的具体数值，只是在把图中的每一步逐项展开写清楚。
