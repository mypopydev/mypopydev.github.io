...
但在所有噪音之下，同一组概念反复出现。一个工具叫它 Skill，另一个叫它 Rule。大多数时候，它们解决的是同一个底层问题。一旦你理解了这些概念，本周哪个工具在热搜上根本不重要。读完本文，你就能读懂任何一篇 智能体 文章或资讯推送，看清它实际覆盖了哪些概念，而不再每次有新东西发布就觉得自己落后了。你可以看着任何一个新智能体工具，快速看懂它到底在干什么。
...
**什么时候不用？** 简单任务用简单方案。格式化日期、转换 JSON、重命名文件——一个普通 prompt 或小脚本更好。因为使用智能体是有成本的。每个循环花时间，每次工具调用花钱。循环越长，越难预测智能体会做什么。
...
不过模型通常不会自己直接跑完一切。模型周围通常有一层 harness：它接收工具调用、检查是否合法、安全运行、再把结果返回。你可以把这层想成智能体周围的 harness。
...
子智能体则要容易管理得多：每个都从全新的上下文窗口开始，父智能体只传入它需要的那部分。如果父智能体得传超过几段的内容，那两者的拆分大概率本身就错了。
...
**保持配置文件短小、精炼、实用。** 控制在 100 行以内。像对待代码一样对待它——改了要审查，智能体反复犯错时改进，没用的规则删掉。
...
没有清晰的流程，智能体可能毫无章法地工作——有时直接跳进代码、跳过测试、改完再解释为什么对（即使结果并不好）。
...
**规则**：保持上下文精简。配置文件短。工作流文件具体。删掉任何不能帮智能体做更好决策的东西。**每个 token 都该物有所值。**
...
MCP 是连接智能体与外部工具和服务的一种标准方式。基本概念：工具以智能体已经理解的格式暴露自己，而不是为每个工具、每个智能体写定制胶水代码。
...
**模式很简单**：智能体已经擅长写代码。Skill 或 MCP 则负责教它生成哪种可视化格式。把智能体从编码助手，变成可视化输出生成器。
...
**但有限制**：如果 `MEMORY.md` 太长，会有和超大配置文件一样的问题——占上下文、加噪音。
...
当多个智能体并行运行时，你需要一个管理层面来协调工作。启动智能体容易，协调它们才是难点。
...
智能体不总是小心的。它们是问题解决者，有时会走捷径——而且是坏的那种：命令失败就试危险修复、测试一直失败就删断言、依赖装不上就试随机安装脚本、Git 推不上去就找绕过方式。
...
日志是可观测性的基础层。在你能追踪、回放或测量任何东西之前，你需要的是发生了什么事的原始记录。

**好日志至少捕获**：每个模型调用（prompt、响应、延迟、token 用量、模型版本）、每个工具调用（工具名、参数、结果、延迟）、每个错误、一个把整个运行串起来的 session ID。

**不要过度设计**。简单结构化日志通常最好。JSON Lines 好用——每个事件变成一条清晰记录，文件容易搜索、存储和后续处理。
