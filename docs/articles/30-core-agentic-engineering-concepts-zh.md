...
- **Figma MCP**：智能体读真实设计数据（布局、组件、间距、变量、样式），生成代码。你不用文字描述 UI 或贴截图，直接把智能体指向一个 Figma 画框，它就能理解真实设计并据此生成代码；有些流程里它还能把改动推回 Figma 画布。
- **frontend-slides**：从 prompt 生成完整 HTML 演示文稿，产出一个包含 HTML、CSS、JavaScript、可在浏览器直接打开的自包含文件。
- **draw.io**：基于结构化 XML，智能体可以从 Terraform 仓库生成架构图；若接入 CI，你的图能更贴近真实系统，而不是慢慢过时。
- **Remotion**：用代码创建视频；懂 Remotion 最佳实践的智能体，就能像生成幻灯片或架构图一样，从指令生成视频文件。
...
**升级方案**：可搜索记忆。像 episodic memory 这样的工具可以索引过往对话、创建 embedding、让智能体在需要时搜索旧会话。这很有用，因为文档通常告诉你"决定了什么"，而会话历史往往告诉你"为什么这么决定"。
...
- **Conductor**：给 Claude Code 和 Codex 一个并行会话的统一 UI，每个智能体在隔离工作区工作，内置 diff 查看器帮你对比和合并改动。
- **JetBrains Air**：在 JetBrains 生态内类似思路，用 Docker 容器或 Git worktrees 隔离任务。
- **Vibe Kanban**：看板方式，把工作拆成卡片，分配给智能体，可视化追踪进度。
- **Cline Kanban**：跨 Claude Code、Codex、Cline 等多种智能体工作，还带 auto-commit、依赖感知的并行等能力。
- **Paperclip**：更野心勃勃——试图做全 AI 运营公司的编排层，有组织架构图、任务委托、预算、人工审批，对个人开发者可能过重。
...
**AST-grep** 让你针对那个结构写规则。这对 AI 写的代码特别重要——LLM 不总是犯明显的错。它们经常写出看起来干净、通过格式检查、通过类型检查、有时甚至通过测试的代码。但底下的模式，仍然可能是错的。这同样适用于被吞掉的异常（swallowed exceptions）或裸 except 块（bare except）这类会过度捕获的块。
