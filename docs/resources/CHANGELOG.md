...
本次完成仓库的"站点化清理"，消除根目录与 `docs/` 之间的内容重复，确保 Single Source of Truth：

- 删除根目录 15 个 `*_demo_zh.md` 旧副本，正式版统一位于 `docs/series/agentic-engineering-patterns/`
- 将根目录 5 篇研究报告迁入 `docs/resources/research-reports/`，新增索引页并纳入站点导航
- 删除根目录重复的 `BILINGUAL_LINK_MAP.md`、`agentic_engineering_translation_glossary_style_guide_template.md` 和 `CHANGELOG.md`（正式版统一位于 `docs/resources/`）
- 重写 `README.md`：所有内容链接改为指向 `docs/` 下的正式版路径，定位调整为仓库入口导航而非内容副本
- 将 `site/` 构建产物从版本控制中移除并加入 `.gitignore`
- 同步 `docs/resources/CHANGELOG.md` 为最新版本
